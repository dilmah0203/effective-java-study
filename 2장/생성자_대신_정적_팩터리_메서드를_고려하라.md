...
`Character` 클래스는 상위 클래스로, 특정 캐릭터의 기본 속성(힘, 민첩, 지능)을 나타냅니다. 또한 정적 팩토리 메소드 `makeCharacter()`를 통해 `Character` 객체를 생성합니다. `Wizard`와 `Warrior` 클래스는 하위 클래스로 `makeWizard()`와 `makeWarrior()` 메소드를 제공하여 객체를 생성합니다.

정적 팩토리 메소드를 사용하면 메소드 호출을 통해 얻을 객체의 인스턴스를 선택 가능하게 해줍니다.(다형성) 
...
`Character` 클래스는 `makeCharacter()` 메소드로 객체를 생성합니다. 이 메소드는 `type`을 매개변수로 받아 객체의 클래스를 결정합니다. 즉, 반환될 객체가 런타임에 동적으로 결정됩니다.
